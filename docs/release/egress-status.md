...
The separate [browser runtime qualification](browser-runtime-status.md) now exercises the browser-only CONNECT proxy, a certificate-validated synthetic login, HTTP redirect and authenticated page read. Its fixed local fixture mapping is compiled only into the diagnostic executable.

U2/U9 remain open. The gateway carries opaque TLS; it does not inspect HTTP authority inside an established tunnel. Qualification still requires integrated browser origin checks, disabled HTTP/2 coalescing and QUIC, redirect/WebSocket/popup attack tests, controlled DNS rebinding, independent crash/watchdog tests, connection budgeting in the supervisor, and production packaging. No real credential has been used.
