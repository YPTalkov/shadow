...
The counts above describe earlier UI checkpoints. Sources, guided Recovery, mirrored-account restrictions, VM enrollment/requests, teardown and catalog pagination are now integrated and exercised by the [packaged owner harness and VM tests](package-status.md). Editor handoff revokes active authority before releasing the worker. Keyboard/VoiceOver and the owner's walkthrough remain open; this report does not mark U6 or the release complete.
