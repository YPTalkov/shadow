...
- The exact production browser image boots Chromium and its private display, then stops within the host-silence limit. It stopped after 3.37 seconds in the current measured run (`production-browser-results.json`): the worker's three-second silence bound fired before PID 1's independent ten-second maximum. No fixture CA or gateway exception was present in this run.
