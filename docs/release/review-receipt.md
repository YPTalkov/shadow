...
Hosted runs exposed a test timing error in `activeTunnelStopsOnRevocation`: its elapsed-time assertion included the delay before the awaiting test task resumed. The test now timestamps the forwarding thread's denial, still requires termination after revocation within one second, and still verifies that later bytes cannot cross. The production gateway is unchanged. The full local 77-test native suite passes with the corrected measurement.

A parallel hosted run also reached the fixture's one-second socket read before its forwarding task started. The fixture now awaits a startup signal before the handshake and joins the forwarding task on setup failure before closing descriptors. The handshake and revocation limits are unchanged. These repairs change the test's scheduling and measurement, not the gateway's authority checks.
