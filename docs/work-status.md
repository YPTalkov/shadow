...
| U8 | In progress | Shared contracts and Linux Codex MCP pass. Host ChatGPT device sign-in, Keychain custody/refresh and native controls now pass 16 synthetic native checks; live sign-in remains pending. Completing production agent VM controls, then protected source jobs. |
