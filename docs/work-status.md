...
| U8 | In progress | Shared contracts, host ChatGPT device sign-in and native production VM controls implemented. GPT-6 Responses Lite/code tools, selected-model/request limits, MCP, task teardown and lock during an active model request pass synthetic qualification; see `docs/release/agent-runtime-status.md`. Live sign-in, combined two-VM tasks and protected connector refresh jobs remain. |
